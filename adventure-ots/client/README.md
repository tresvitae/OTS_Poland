...
5. **Smoke-test the bundle** — Run `pwsh -File scripts/test-client.ps1 -EnableDebug -TimeoutSeconds 20` from the repo root. The helper in [scripts/test-client.ps1](../../scripts/test-client.ps1) extracts the latest zip to a temp folder, streams OTClient stdout/stderr (fatal module issues, GPU info, etc.), and optionally keeps the workspace with `-KeepExtracted` for manual inspection. Pass extra flags via `-ClientArgs` (e.g., `-ClientArgs "--force-opengl"`) or fail builds decisively with `-FailOnTimeout`.
